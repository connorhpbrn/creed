...
const sections: DocsSection[] = [
  ...
  },
  {
    ...
    paragraphs: [
      "Creed MCP uses OAuth, so there is nothing to copy. Add your installation's MCP URL to an agent as a custom connector. The agent opens a browser, you approve it from an authorised owner browser, and it stays connected. The exact URL is on the Connections page.",
    ...
    bullets: [
      ...
      "The first time the agent calls Creed it runs the OAuth flow and opens a browser. Approve from your authorised owner session. Tokens refresh automatically after that.",
      ...
      "If anything is unclear during setup, return to this guide and follow the connection steps once.",
    ],
  },
  {
    id: "troubleshooting",
  ...
  },
  {
    ...
    paragraphs: [
      "Creed can score how useful your profile is, section by section, and surface where to sharpen it. In Creed Open, quality analysis runs on the OpenRouter API key you add in Settings.",
  ...
  },
  {
    ...
    bullets: [
      ...
      "Your call on AI spend. AI features run on your own OpenRouter key. Your context is only sent to the model to run the feature you asked for.",
      "Secrets are encrypted. API keys and connection tokens are stored with AES-256-GCM, never in plain text.",
      "The owner session can only read its installation's data. Supabase row-level security remains enabled on user-scoped tables.",
      "Hidden sections never leave the app. Set a section to hidden and it is dropped from the agent payload entirely.",
      "You control deletion and backups through Creed and your Supabase project.",
    ],
  },
];

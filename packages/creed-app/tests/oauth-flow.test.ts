...
test("OAuth token lifecycle is retryable without exposing partial successors", () => {
  ...
  const migration = readFileSync(
    new URL(
      "../../../apps/cloud/supabase/migrations/20260815193116_harden_oauth_lifecycle.sql",
      import.meta.url,
    ),
    "utf8",
  );
  ...
  assert.match(migration, /unique index if not exists oauth_tokens_authorization_code_hash_idx/i);
  assert.match(migration, /unique index if not exists oauth_tokens_parent_token_id_idx/i);

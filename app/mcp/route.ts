    ...
    inputSchema: {
      ...
      properties: {
        ...
        contentMarkdown: {
          type: "string",
          description:
            "Document body in Markdown. Body only - set properties via the structured fields below, not YAML frontmatter. Supports rich components: `##`/`###` headings, bullet + numbered lists, `>` callouts, `---` dividers, inline `#tags`, fenced code blocks, GFM pipe tables (`| A | B |` over a `| --- | --- |` row), ```mermaid diagrams, and document references (`[[doc:SLUG]]` / `[[folder:SLUG]]` inline chips, `![[doc:SLUG]]` on its own line for a full-width card). A `##` heading starts a section; nest a subsection with `## Name <!-- creed:depth=1 -->` (max depth 2). Use a table to compare items across shared attributes, and a mermaid flowchart/sequence/ER/journey diagram instead of deeply nested bullets or step lists for branching flows.",
        },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        contentMarkdown: {
          type: "string",
          description:
            "Full replacement document body in Markdown. Body only - do not include YAML frontmatter; change properties with creed_update_document_metadata. Supports rich components: `##`/`###` headings, bullet + numbered lists, `>` callouts, `---` dividers, inline `#tags`, fenced code blocks, GFM pipe tables (`| A | B |` over a `| --- | --- |` row), ```mermaid diagrams, and document references (`[[doc:SLUG]]` / `[[folder:SLUG]]` inline chips, `![[doc:SLUG]]` on its own line for a full-width card). A `##` heading starts a section; nest a subsection with `## Name <!-- creed:depth=1 -->` (max depth 2). Use a table to compare items across shared attributes, and a mermaid flowchart/sequence/ER/journey diagram instead of deeply nested bullets or step lists for branching flows.",
        },
      },
